...
function TiltCard({ item }) {
  ...
  function onMouseMove(e) {
    ...
    el.style.transform = `translateY(-16px) scale(1.02) rotateY(${px * 22}deg) rotateX(${-py * 22}deg)`;
    if (glowRef.current) {
      glowRef.current.style.background = `radial-gradient(220px circle at ${(px + 0.5) * 100}% ${(py + 0.5) * 100}%, rgba(0,255,136,.25), transparent 70%)`;
  ...
  return (
    <div className="c-slide">
      <div className="v-card" ref={cardRef} onMouseMove={onMouseMove} onMouseLeave={onMouseLeave}>
        <div className="v-glow" ref={glowRef} />
